...
\begin{tikzpicture}
	...
	\foreach \xoff/\opa in {-2.3/1, 2.3/0.3}
	{
		...
		\pic[transform shape, opacity=\opa, local bounding box=detector, rotate=-90,scale=0.1] at (\xoff, -4) {detector};
		\draw [
	...
	\draw [-latex, thick, dashed] (-2, -4) -- ++(4, 0);
	\centerarc[-latex, thick, dashed](0,0)(125:130:4.7);
	\rotatebox{15}{
	\foreach \xoff in {-2.3, 2.3}
	{
		\pic[rotate=-90,opacity=0.3, local bounding box=source, scale=0.2] at (\xoff, 4) {source};
		\pic[rotate=-90,transform shape, opacity=0.3, local bounding box=detector, scale=0.1] at (\xoff, -4) {detector};
		\draw [
			-latex, decorate, decoration={snake, amplitude=0.05cm, segment length=5pt, post length=1mm},%
